...
public class CityMap extends JPanel {
    ...
    public CityMap(Point[] listPoint, Double offset) {
    ...
    */
    ...
    public void paint(Graphics g) {
     try {
        ...
        if (points != null){       
    	//	System.out.println("ve: "+points.length+"-"+os); 	
			for (int i = 0; i < points.length; i++) {
				try {
	                final BufferedImage img =ImageIO.read( this.getClass().getResourceAsStream("/img/maker.png")); 
	    			g2.drawImage(img, (int) (points[i].x*os), (int) (points[i].y*os), null);//(400, 300, 5, 5);
				} catch (Exception e) {
